...
import { coerceRoomsFromImport } from "@/src/model/migrate";
...
import { ImportRoomModal } from "@/src/ui/ImportRoomModal";
...
/** "house" appends every room from the file to the current home; "room" appends just one, prompting to pick when the file holds several. */
type ImportMode = "house" | "room";
...
export function Toolbar() {
  ...
  const importRooms = useStore((s) => s.importRooms);
  const resetHome = useStore((s) => s.resetHome);
  const selectedRoomId = useStore((s) => s.selectedRoomId);

  const fileInputRef = useRef<HTMLInputElement>(null);
  const importModeRef = useRef<ImportMode>("house");
  const [status, setStatus] = useState<string | null>(null);
  const statusTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [printOptions, setPrintOptions] = useState<PrintOptions>(DEFAULT_PRINT_OPTIONS);
  const [roomPickerRooms, setRoomPickerRooms] = useState<Room[] | null>(null);
  ...
  function handleImportFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    file.text().then((text) => {
      ...
      try {
        ...
        flashStatus("That file isn't valid JSON");
        return;
      }
      const rooms = coerceRoomsFromImport(parsed);
      if (rooms.length === 0) {
        flashStatus("No rooms found in that file");
        return;
      }
      if (mode === "house") {
        importRooms(rooms);
        flashStatus(rooms.length === 1 ? "Imported 1 room" : `Imported ${rooms.length} rooms`);
        return;
      }
      // mode === "room": expected to be a single room, but the file may turn
      // out to hold several (e.g. a full house plan uploaded by mistake) —
      // let the user pick which one instead of silently grabbing the first.
      if (rooms.length === 1) {
        importRooms(rooms);
        flashStatus("Imported room");
      } else {
        setRoomPickerRooms(rooms);
      }
    });
    e.target.value = "";
  }

  function handleRoomPicked(room: Room) {
    importRooms([room]);
    setRoomPickerRooms(null);
    flashStatus("Imported room");
  }
  ...
  return (
    <>
          ...
          sections={[
            {
              items: [
                {
                  key: "import-house",
                  label: "Import house plan",
                  onSelect: () => triggerImport("house"),
                },
                {
                  key: "import-room",
                  label: "Import room",
                  onSelect: () => triggerImport("room"),
                },
              ],
            },
          ]}
      ...
      {roomPickerRooms && (
        <ImportRoomModal
          rooms={roomPickerRooms}
          units={units}
          onSelect={handleRoomPicked}
          onClose={() => setRoomPickerRooms(null)}
        />
      )}
    </>
  );
